"""
Nostalgia Classifier - Inference
Classify new political ads using the trained model.

Usage:
    # Single ad
    python inference.py --model_dir models/nostalgia_classifier --text "We will make America great again"
    
    # CSV file
    python inference.py --model_dir models/nostalgia_classifier --csv_path new_ads.csv --output_path predictions.csv
"""
...
def predict_single(text, model, tokenizer, device, max_length=256):
    """Predict nostalgia score for a single text."""
    encoding = tokenizer(
        text,
        truncation=True,
        padding='max_length',
...
def predict_batch(texts, model, tokenizer, device, batch_size=32, max_length=256):
    """Predict nostalgia scores for a batch of texts."""
    results = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv_path', type=str, help='CSV file with texts to classify')
    parser.add_argument('--text_column', type=str, default='Transcript', help='Column name for text')
